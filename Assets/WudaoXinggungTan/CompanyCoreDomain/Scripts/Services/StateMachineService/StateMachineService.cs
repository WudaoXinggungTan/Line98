...
using UnityEngine;
using WudaoXinggungTan.CompanyCoreDomain.Scripts.MVC.LoadingScreen;
    ...
    public class StateMachineService : IStateMachineService
    {
        ...
        private async Awaitable SwitchStateAsync(IGameState newState)
        {
            try
            {
                ...
                {
                    MyLogService.LogError("No state to switch from, need to initialize a game state first!");
                    return;
                }
            ...
            {
                MyLogService.Log("Switching state operation was cancelled");
            }
            catch (Exception e)
            {
                MyLogService.LogError(e.Message);
                throw;
            }
        }
    }
