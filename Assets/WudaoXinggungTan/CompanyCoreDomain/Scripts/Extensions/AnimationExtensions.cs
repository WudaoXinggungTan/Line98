...
namespace WudaoXinggungTan.CompanyCoreDomain.Scripts.Extensions
{
    public static class AnimationExtensions
    {
        public static async Awaitable PlayAsync(this Animation animation, string clipName, CancellationToken cancellationToken = default, AwaitableCompletionSource completionSource = null)
        {
            if (animation == null)
            {
                MyLogService.LogError($"animation for clip {clipName} is null");
                return;
            }
            ...
            if (clip == null)
            {
                MyLogService.LogError($"clip {clipName} is null");
                return;
            }
